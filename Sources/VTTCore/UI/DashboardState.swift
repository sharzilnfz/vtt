...
public enum VTTAppTheme: String, CaseIterable, Identifiable, Sendable {
    case dark
    case light
    case system
...
public final class DashboardState: ObservableObject {
    ...
    @Published public var activeTheme: VTTAppTheme = .dark
    ...
    public init(
        ...
        let savedTheme = UserDefaults.standard.string(forKey: "vtt.theme") ?? "dark"
        self.activeTheme = VTTAppTheme(rawValue: savedTheme) ?? .dark
    ...
    public func setTheme(_ theme: VTTAppTheme) {
        activeTheme = theme
        UserDefaults.standard.set(theme.rawValue, forKey: "vtt.theme")
    }
    ...
    public func addActivity(title: String, subtitle: String? = nil, kind: ActivityItem.ActivityKind = .transcript) {
        guard kind == .transcript || kind == .refinement else { return }
