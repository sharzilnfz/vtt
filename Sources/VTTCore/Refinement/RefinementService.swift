...
public final class RefinementService: RefinementServiceProtocol {
    private let config: RefinementEndpointConfig?
    ...
    public func refine(rawText: String) async -> String {
        ...
        let snapshot: RefinementEndpointConfig?
        if let settings, let config = await settings.config {
            snapshot = config
        } else if let config {
            snapshot = config
        } else {
            snapshot = RefinementEndpointConfig()
        }
        guard let config = snapshot,
              let endpoints = try? GatewayEndpoints(config.url.absoluteString),
