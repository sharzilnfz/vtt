...
public final class AudioCaptureService: @unchecked Sendable, AudioCaptureServiceProtocol {
    ...
    public func startCapture(onLevelUpdate: (@Sendable (Float) -> Void)? = nil) throws {
        ...
        guard let targetFormat = AVAudioFormat(
            ...
            throw NSError(domain: "UtterAudio", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create target audio format"])
        }

        guard let converter = AVAudioConverter(from: inputFormat, to: targetFormat) else {
            throw NSError(domain: "UtterAudio", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to create audio converter"])
        }
